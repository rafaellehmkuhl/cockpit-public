...
export function monitorElementAndStopCapture(elementId) {
  ...
  // Function to check if element is hidden and stop capture
  const checkAndStopCapture = () => {
    if (!isCapturing) {
      cleanup()
      return
    }
    ...
    if (isHidden) {
      console.log(`Element "${elementId}" is now hidden, stopping console capture`)
      stopConsoleCapture()
      cleanup()
    }
  }
  ...
  const intersectionObserver = new IntersectionObserver(
    (entries) => {
      ...
      entries.forEach((entry) => {
        // If element is not intersecting (not visible), stop capture
  ...
  // Watch for direct style attribute changes on the element
  ...
  // Periodic check as a fallback (every 1 second)
  intervalId = setInterval(checkAndStopCapture, 1000)

  // Also stop capture when the page is about to unload
  const handleUnload = () => {
    if (isCapturing) {
      stopConsoleCapture()
    }
