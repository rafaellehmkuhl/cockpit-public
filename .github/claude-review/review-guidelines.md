...
- Via Bash you have `jq`, `rg`, `grep`, `cat`, `head`, `tail` and `wc`. To read a specific line range, use the `View` tool with an offset: `sed` is not on the allowlist, and every denied command still costs you one of your turns. There is no network access and no `gh`: everything you need about the PR is already on disk.
...
- Everything else goes inside a `<details>` block, collapsed by default: the Change map, each section with findings, the clean sections, and the comparison against the previous round.
...
- Still perform the full analysis for every section. Only write out the body of a section that has at least one finding; each of those gets its own `<details>`.
...
- The Change map always has a body, inside its own `<details>`. Section 2 (Persistence & User Data) also always has a body whenever the PR touches persisted data, because its inventory must be written out even when every entry is fine.
...
- `#` is the finding id, so the reader can find it in the detail below.
...
the author's argument in one line, a checkbox per option, and the command that records the decision:
...
- [ ] Accept the argument and close the finding
- [ ] Ask for the change anyway
Record it with `/resolve 1.4 <reason>` (ticking a box leaves a record, but only the command closes it).
```
...
> [!CAUTION]
> **:no_entry: DO NOT MERGE** — blocking: 1.1, 1.2. 6 open, 0 closed.
...
### :raising_hand: Decisions for a human      (only when a finding is disputed)

<one block per disputed finding, as specified in section 0>

<details>                            (only when a previous review exists)
<summary>Since round $((SEQ-1)) — 5 closed, comparing abc1234 → def5678</summary>
...
<one such block per section that has findings, in section order>
...
Two short-circuits. Both still emit the marker, the verdict alert, the footer and the full ledger,
and both skip everything else:

- If `pr.diff` is empty or missing, say so (with an empty `[]` ledger) and stop.
