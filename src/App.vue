...
const closeSubMenuModal = (): void => {
  console.info(`[UserAction] Closed '${interfaceStore.currentSubMenuComponentName ?? 'settings'}' panel`)
...
const toggleMainMenu = (): void => {
  if (interfaceStore.isMainMenuVisible) {
    ...
    console.info('[UserAction] Opened main menu')
...
const closeMainMenu = (): void => {
  console.info('[UserAction] Closed main menu')
...
const handleEscKey = (event: KeyboardEvent): void => {
  if (event.key === 'Escape' && interfaceStore.isMainMenuVisible) {
    console.info('[UserAction] Closed main menu (Escape key)')
    closeMainMenu()
  }
}
...
const bottomBarToggleCallbackId = registerActionCallback(availableCockpitActions.toggle_bottom_bar, () => {
  ...
  console.info(`[UserAction] Toggled bottom bar visibility to ${showBottomBarNow.value}`)
})
const topBarToggleCallbackId = registerActionCallback(availableCockpitActions.toggle_top_bar, () => {
  ...
  console.info(`[UserAction] Toggled top bar visibility to ${showTopBarNow.value}`)
})
onBeforeUnmount(() => {
