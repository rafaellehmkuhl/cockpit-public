...
/**
 * The live concatenation process
 */
interface LiveConcatProcess {
  ...
  id: string
  /**
   * The FFmpeg process (not used for binary concatenation)
   */
  ffmpegProcess: ChildProcess
  /**
   * The input pipe of the final video
   */
  inputPipe: string
  /**
...
const processZipFile = async (
  ...
  console.debug(`Trying to process ZIP file '${zipFilePath}' to ${tempDir}.`)
  ...
  try {
    ...
    const { id: processId } = await startLiveVideoConcat(firstChunkPath, outputPath)
...
export const setupLiveVideoService = (): void => {
   ...
   */
  ipcMain.handle('start-live-video-concat', async (_, firstChunkPath: string, outputPath: string) => {
    try {
      const result = await startLiveVideoConcat(firstChunkPath, outputPath)
      return result
    } catch (error) {
