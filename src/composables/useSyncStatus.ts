...
interface SyncState {
  ...
  currentStep: string
  /**
   * The settings that were resolved during the sync
   */
  resolvedSettings: SyncSettingDetail[]
  /**
   * The progress of the push operation
   */
  pushProgress: {
    /**
  ...
  /**
   * The username of the user that triggered the sync (if any)
   */
  currentUser: string
  /**
   * The ID of the vehicle that triggered the sync (if any)
   */
  ...
  showDialog: boolean
  /**
   * Whether the auto close feature is active
   */
  autoCloseActive: boolean
  /**
   * The progress of the auto close feature
   */
  ...
  const handleSyncEvent = (event: SyncStatusEvent): void => {
    switch (event.type) {
      case 'sync-started':
        ...
        state.pushProgress = { pushed: 0, skipped: 0, total: 0 }
      ...
      case 'push-started':
        state.pushProgress = { pushed: 0, skipped: 0, total: event.totalKeys }
        break
