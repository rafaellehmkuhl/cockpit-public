...
export const useWidgetManagerStore = defineStore('widget-manager', () => {
  ...
  watch(widgetsWithOpenConfigMenu, (newHashes, oldHashes) => {
    newHashes
      ...
      .forEach((hash) => console.info(`[UserAction] Opened configuration of widget '${widgetNameByHash(hash)}'`))
    oldHashes
      .filter((hash) => !newHashes.includes(hash))
      .forEach((hash) => console.info(`[UserAction] Closed configuration of widget '${widgetNameByHash(hash)}'`))
  })
  ...
  watch(miniWidgetsWithOpenConfigMenu, (newHashes, oldHashes) => {
    newHashes
      .filter((hash) => !oldHashes.includes(hash))
      .forEach((hash) =>
        console.info(`[UserAction] Opened configuration of mini-widget '${getElementByHash(hash)?.component ?? hash}'`)
      )
    oldHashes
      .filter((hash) => !newHashes.includes(hash))
      .forEach((hash) =>
        console.info(`[UserAction] Closed configuration of mini-widget '${getElementByHash(hash)?.component ?? hash}'`)
      )
  })
