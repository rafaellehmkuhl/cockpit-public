...
import { DataLakeVariableConfig } from '@/libs/actions/data-lake'
...
interface ExtrasJson {
  ...
  /**
   * A list of actions that the extra json contains. src/types/cockpit-actions.ts
   */
  actions: ActionConfig[]
  /**
   * A list of data lake variables that the extra json contains. src/types/data-lake.ts
   */
  data_lake_variables: DataLakeVariableConfig[]
}
...
export const getDataLakeVariablesFromBlueOS = async (): Promise<DataLakeVariableConfig[]> => {
  const vehicleStore = useMainVehicleStore()
  ...
  const services = await getServicesFromBlueOS(vehicleStore.globalAddress)
  const dataLakeVariables: DataLakeVariableConfig[] = []
  await Promise.all(
    services.map(async (service) => {
      const extraJson = await getExtrasJsonFromBlueOsService(vehicleStore.globalAddress, service)
      if (extraJson !== null) {
        dataLakeVariables.push(...extraJson.data_lake_variables)
      }
    })
  )

  return dataLakeVariables
}
