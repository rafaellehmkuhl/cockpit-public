...
const allVariables = computed(() => {
  const variables = getAllCockpitActionVariablesInfo()
...
const setupVariableListeners = () => {
  const variables = getAllCockpitActionVariablesInfo()
  Object.keys(variables).forEach((id) => {
    currentValues.value[id] = getCockpitActionVariableData(id)

    listenCockpitActionVariable(id, (value) => {
      currentValues.value[id] = value
    })
  })
}

const cleanupVariableListeners = () => {
  const variables = getAllCockpitActionVariablesInfo()
  Object.keys(variables).forEach((id) => {
    unlistenCockpitActionVariable(id)
  })
}
...
const openEditDialog = (id: string): void => {
  const variable = getCockpitActionVariableInfo(id)
  if (!variable) return
  ...
  newVariable.value = {
    ...
    type: variable.type,
    description: variable.description || '',
    value: getCockpitActionVariableData(id),
  }
  variableDialog.value.show = true
}
...
const saveVariable = (): void => {
  const variable = new CockpitActionVariable(
  ...
  if (editMode.value) {
    updateCockpitActionVariableInfo(variable)
    if (newVariable.value.value !== undefined) {
      setCockpitActionVariableData(variable.id, newVariable.value.value)
    }
  } else {
    createCockpitActionVariable(variable, newVariable.value.value)
    listenCockpitActionVariable(variable.id, (value) => {
...
const deleteVariable = (id: string): void => {
  ...
  deleteCockpitActionVariable(id)
}
