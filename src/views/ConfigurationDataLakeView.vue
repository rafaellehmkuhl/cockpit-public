              ...
              >
                <template #headers>
                  <tr>
                    ...
                    <th class="text-left">
                      <p class="text-[16px] font-bold">Expression</p>
                    </th>
                    <th class="text-right">
                ...
                <template #item="{ item }">
                  <tr>
                    ...
                    <td>
                      <div class="flex items-center justify-left rounded-xl mx-1">
                        <p class="whitespace-nowrap overflow-hidden text-overflow-ellipsis">{{ item.expression }}</p>
                      </div>
                    </td>
...
const editingFunction = ref<TransformingFunction | null>(null)
...
const copyVariableId = (id: string, name: string): void => {
  copyToClipboard(id)
  showSnackbar({ message: `Variable ID for "${name}" copied to clipboard`, variant: 'success', duration: 2000 })
 ...
 * @param func - The function to edit
 */
...
const handleDeleteFunction = (func: TransformingFunction): void => {
  ...
  const index = transformingFunctions.value.findIndex((f) => f.name === func.name)
  if (index !== -1) {
    transformingFunctions.value.splice(index, 1)
  }

  // Delete the function and clean up
...
const saveTransformingFunction = (): void => {
  if (!newFunction.value.name || !newFunction.value.expression) return

  if (editingFunction.value) {
    // Remove old function
    handleDeleteFunction(editingFunction.value)
  }

  // Create and add new function
  const func = createTransformingFunction(
    newFunction.value.name,
    newFunction.value.expression,
    Object.keys(variablesMap.value)
  )
  transformingFunctions.value.push(func)
...
onMounted((): void => {
  ...
  const unsubscribe = listenToVariableChanges((variables) => {
...
onUnmounted(() => {
  // Clean up all transforming functions
  transformingFunctions.value.forEach((func) => {
