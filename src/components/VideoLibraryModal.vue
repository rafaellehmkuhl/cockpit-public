...
const openPicInFullScreen = async (picture: SnapshotLibraryFile): Promise<void> => {
  console.info(`[UserAction] Opened picture '${picture.filename}' in fullscreen`)
...
const deletePictures = async (pictureFileName?: string): Promise<void> => {
  try {
    console.info(`[UserAction] Deleted ${pictureFileName ? `picture '${pictureFileName}'` : 'selected pictures'}`)
...
const downloadPictures = async (pictureFileName?: string): Promise<void> => {
  try {
    console.info(`[UserAction] Downloaded ${pictureFileName ? `picture '${pictureFileName}'` : 'selected pictures'}`)
...
const closeModal = (): void => {
  console.info('[UserAction] Closed Video Library')
...
const selectTab = (tabName: string): void => {
  console.info(`[UserAction] Switched Video Library to '${tabName}' tab`)
  currentTab.value = tabName
}

const toggleSelectionMode = (): void => {
  isMultipleSelectionMode.value = !isMultipleSelectionMode.value
  console.info(`[UserAction] ${isMultipleSelectionMode.value ? 'Enabled' : 'Disabled'} multiple selection mode`)
...
const selectAllVideos = (): void => {
  console.info('[UserAction] Selected all videos')
...
const discardVideosAndUpdateDB = async (videos?: VideoLibraryFile[]): Promise<void> => {
  console.info(`[UserAction] Deleted ${(videos || selectedVideos.value).length} video(s) from the library`)
  deleteButtonLoading.value = true
...
const selectAllPictures = (): void => {
  console.info('[UserAction] Selected all pictures')
...
const handleProcessVideoChunksZip = async (): Promise<void> => {
  console.info('[UserAction] Started processing a video chunks ZIP')
