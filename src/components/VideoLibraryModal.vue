                      ...
                      <div class="space-y-3">
                        ...
                        >
                          ...
                          <div class="flex items-center gap-2 ml-4">
                            <!-- Process button for unprocessed WebM files -->
                            <v-btn
                              v-if="!video.isProcessed && video.fileName.endsWith('.webm') && !isRecordingOngoing()"
                              icon
                              variant="outlined"
                              size="small"
                              :disabled="
                                showOnScreenProgress || isPreparingDownload || processingVideoFiles.has(video.fileName)
                              "
                              @click.stop="processWebmVideo(video)"
                            >
                              <v-tooltip open-delay="500" activator="parent" location="bottom">
                                {{ processingVideoFiles.has(video.fileName) ? 'Processing...' : 'Process video' }}
                              </v-tooltip>
                              <v-icon>{{
                                processingVideoFiles.has(video.fileName) ? 'mdi-loading mdi-spin' : 'mdi-file-cog'
                              }}</v-icon>
                            </v-btn>
                            <v-btn
                              icon
                ...
                <template v-if="currentVideoSubTab === 'temporary'">
                  ...
                  <div v-else class="flex flex-col h-full">
                    ...
                    <div v-else class="flex flex-col h-full">
                      <!-- Empty State Message -->
                      <div class="flex justify-center items-center flex-1 text-xl text-center">
                        {{ chunkLoadingData ? 'Loading' : 'No temporary chunks found' }}
                      </div>
...
const currentVideoSubTab = ref(isElectron() ? 'processed' : 'temporary')
...
const processingVideoFiles = ref<Set<string>>(new Set())
...
/**
 * Process a WebM video file to convert it to MP4
 * @param {VideoLibraryFile} video - The video to process
 */
const processWebmVideo = async (video: VideoLibraryFile): Promise<void> => {
  if (!isElectron() || !window.electronAPI?.processStandaloneWebm) {
    openSnackbar({ message: 'Video processing is only available in the desktop version.', variant: 'error' })
    return
  }

  try {
    processingVideoFiles.value.add(video.fileName)
    openSnackbar({ message: `Processing ${video.fileName}...`, variant: 'info' })

    await window.electronAPI.processStandaloneWebm(video.fileName)

    openSnackbar({ message: 'Video processed successfully!', variant: 'success' })

    // Refresh the video list
    await fetchVideosAndLogData()
  } catch (error) {
    const errorMsg = `Failed to process video: ${(error as Error).message ?? error!.toString()}`
    openSnackbar({ message: errorMsg, variant: 'error' })
  } finally {
    processingVideoFiles.value.delete(video.fileName)
  }
}
