<template>
  <div class="flex items-center w-fit min-w-[8rem] max-w-[9rem] h-12 p-1 text-white justify-center">
    ...
    <div class="flex flex-col items-start justify-center ml-1 min-w-[4rem] max-w-[6rem] select-none">
      <div>
        <span class="font-mono text-xl font-semibold leading-6 w-fit">{{ displayValue }}</span>
...
const displayValue = computed(() => round(averageDepth.value, 2).toFixed(2))
watch(displayValue, () => datalogger.updateVariable(DatalogVariable.depth, `${displayValue.value} m`))
</script>
