...
const openVehicleDefaultsImportModal = (): void => {
  console.info('[UserAction] Opened vehicle defaults import dialog')
...
viewRenameDialog.onConfirm(() => {
  console.info(`[UserAction] Renamed view '${viewBeingRenamed.value.name}' to '${newViewName.value}'`)
  store.renameView(viewBeingRenamed.value, newViewName.value)
  newViewName.value = ''
})
viewRenameDialog.onCancel(() => {
  console.info(`[UserAction] Cancelled rename of view '${viewBeingRenamed.value.name}'`)
  newViewName.value = ''
})

const addNewView = (): void => {
  if (!viewRenameDialogRevealed.value) {
    console.info('[UserAction] Added new view')
    store.addView()
    forceUpdate.value++
...
const onWidgetPaletteModeChange = (mode: string): void => {
  console.info(`[UserAction] Switched widget palette to '${mode}' mode`)
}

const renameView = (view: View): void => {
  console.info(`[UserAction] Opened rename dialog for view '${view.name}'`)
...
const toggleViewVisibility = (view: View): void => {
  ...
  console.info(`[UserAction] Set view '${view.name}' visibility to ${view.visible}`)
}
  ...
  showDialog({
    ...
    actions: [
      ...
      },
      {
        text: 'reset',
        action: () => {
          console.info('[UserAction] Reset views group to defaults')
...
const onRegularWidgetDragEnd = (widget: InternalWidgetSetupInfo, event: DragEvent | TouchEvent): void => {
  ...
  if (isWithinMainView) {
    ...
    console.info(`[UserAction] Added widget '${widget.name}' (${widget.component}) to view '${store.currentView.name}'`)
