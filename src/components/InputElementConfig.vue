...
const handleResetVariable = (): void => {
  console.info('[UserAction] Reset data-lake variable on custom element')
...
const openVariableCreationForm = (): void => {
  console.info('[UserAction] Opened new data-lake variable form for custom element')
  openNewDataLakeVariableForm.value = true
}

const openVariableSelector = (): void => {
  console.info('[UserAction] Opened data-lake variable selector for custom element')
...
const deleteParameterFromDataLake = async (): Promise<void> => {
  if (currentElement.value?.options.dataLakeVariable?.name) {
    try {
      console.info(`[UserAction] Deleted data-lake variable '${currentElement.value.options.dataLakeVariable.name}'`)
...
const saveOrUpdateParameter = (): void => {
  ...
  if (
    ...
    console.info(`[UserAction] Created data-lake variable '${newDataLakeVariable.name}' for custom element`)
  ...
  if (futureDataLakeVariable.value && currentElement.value?.options.dataLakeVariable?.name) {
    ...
    console.info(`[UserAction] Updated data-lake variable '${newDataLakeVariable.name}' for custom element`)
...
const addSelectorOption = (key: string): void => {
  if (currentElement.value) {
    console.info('[UserAction] Added selector option to custom element')
...
const removeSelectorOption = (key: string, index: number): void => {
  if (currentElement.value && currentElement.value.options.layout[key]) {
    console.info('[UserAction] Removed selector option from custom element')
...
const deleteElement = (): void => {
  if (currentElement.value) {
    console.info(`[UserAction] Deleted custom widget element '${currentElement.value.component}'`)
    try {
